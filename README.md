...
server on Arch in order to provide secure monitoring of devices on the
home network.

## Why this project?

I wanted to monitor devices on the home network in order to prevent 
any intrusions.There also happened to be an old Thinkpad x230 which 
was not being used since 2020 . So to kill two birds with one stone
...
3)To ensure the server was not accessible from outside the local network we switch off upnp 
and virtual server settings from our router panel and also establish an uncomplicated firewall
(ufw) on our server.
...
3)Logged into router panel and adjusted so upnp and virtual servers weren't enabled as well as assigned a static address
...
6)Install and configure fail2ban to protect SSH access from brute force attacks.(redundant given UFW already restricts to the local subnet but I chose to keep it as good practce).
